...
bool Propagation(NymphPatchEnergyPack& energy, const Mat &src, const Mat &dst, Mat &cor, int patch_radius, int row, int col)
{
    bool flag = false;
    auto& cor_pt = cor.at<Vec2i>(row, col);
    double old_energy = energy.func(energy.param, src, dst, patch_radius, row, col, cor_pt[0], cor_pt[1]);
    ...
    if (col > patch_radius)
    {
        ...
        {
            double from_left_energy = energy.func(energy.param, src, dst, patch_radius, row, col, cor_left[0], cor_left[1] + 1);
    ...
    if (row > patch_radius)
    {
        ...
        {
            double from_top_energy = energy.func(energy.param, src, dst, patch_radius, row, col, cor_top[0] + 1, cor_top[1]);
...
bool RandomSearch(NymphPatchEnergyPack& energy, const Mat &src, const Mat &dst, Mat &cor, int patch_radius, int row, int col, int radius, double alpha)
{
    bool flag = false;
    auto& cor_pt = cor.at<Vec2i>(row, col);
    double old_energy = energy.func(energy.param, src, dst, patch_radius, row, col, cor_pt[0], cor_pt[1]);
    ...
    do
    {
        ...
        double new_energy = energy.func(energy.param, src, dst, patch_radius, row, col, new_row, new_col);
