...
CudaTextureHost::CudaTextureHost(int textureType, int typeSize, const char* name, void* texture) :
...
void CudaTextureHost::initialize(int width, int height, cudaTextureFilterMode filter_mode)
{
    ...
    checkCUDAError("Can't get tex ref for init TEXTURE", _name);
...
void CudaTextureHost::bindTexture()
{
    const textureReference* constTexRefPtr=NULL;
    cudaGetTextureReference(&constTexRefPtr, _texture_name);
    checkCUDAError("Can't get tex ref for bind TEXTURE", _name);
    cudaChannelFormatDesc formatDesc = constTexRefPtr->channelDesc;
...
void CudaTextureHost::unbindTexture()
{
    const textureReference* constTexRefPtr=NULL;
    cudaGetTextureReference(&constTexRefPtr, _texture_name);
    checkCUDAError("Can't get tex ref for unbind TEXTURE", _name);
