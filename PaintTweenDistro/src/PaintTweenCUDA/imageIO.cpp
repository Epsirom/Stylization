...
{
    std::cout << filePath.toAscii().data() << " " << desc._layer_name.toAscii().data() << std::endl;
    ...
    try
    {
        std::cout << qPrintable(remapFilePath(filePath)) << std::endl;
        InputFile file(qPrintable(remapFilePath(filePath)));
        std::cout << "File object ready." << std::endl;

        Box2i dw = file.header().dataWindow();
        width = dw.max.x - dw.min.x + 1;
        height = dw.max.y - dw.min.y + 1;

        std::cout << "DW ready." << std::endl;

        pixels.resize(width*height*desc.numChannels());

        std::cout << "Pixels ready." << std::endl;

        FrameBuffer frameBuffer;

        std::cout << "Target: " << desc.numChannels() << std::endl;

        for(int l= 0; l<desc.numChannels(); l++){
            std::cout << "What happened?" << std::endl;
            std::cout << l << std::endl;
            std::cout << qPrintable(desc._layer_name + "." + desc._channels.at(l)) << " "
                      << (sizeof(float)*desc.numChannels()) << " "
                      << (sizeof(float)*width*desc.numChannels()) << std::endl;
            Slice slice(FLOAT, (char *) (pixels.data()) + l*sizeof(float),
                        sizeof(float)*desc.numChannels(), sizeof(float)*width*desc.numChannels());
            std::cout << "Slice ready." << std::endl;
        ...
        std::cout << "Set frame buffer done." << std::endl;

        file.readPixels (dw.min.y, dw.max.y);

        std::cout << "Read pixels done." << std::endl;
    }
    catch (const std::exception &e)
    {
        qWarning() << e.what();
