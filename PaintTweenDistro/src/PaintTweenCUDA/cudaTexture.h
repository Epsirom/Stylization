...
{  
public:
    CudaTextureHost(int textureType, int typeSize, const char* name, void* texture);
    ~CudaTextureHost();
...
#define TEXTURE_(NAME) \
    NAME ## _TEXTURE

#define CUDA_TEXTURE(TYPE,DATA_TYPE,NAME) \
        CudaTextureHost NAME((TYPE),sizeof(DATA_TYPE), #NAME, TEXTURE_(NAME)); \
        texture<DATA_TYPE,TYPE> TEXTURE_(NAME); \

#define TEXTURE_FETCH1D(NAME,X) tex1D(TEXTURE_(NAME),(X))
#define TEXTURE_FETCH2D(NAME,X,Y) tex1D(TEXTURE_(NAME),(X),(Y))
#define TEXTURE_FETCH3D(NAME,X,Y,Z) tex1D(TEXTURE_(NAME),(X),(Y),(Z))
