...
class CudaImagePyramidHost
{
    public:
        ...
        CudaImagePyramidHost(int textureType, int typeSize, const char *name, void *texture);
...
#define TEXTURE_PYRAMID(NAME) \
    NAME ## _TEXTURE_PYRAMID

#define CUDA_IMAGE_PYRAMID(TYPE,NAME) \
    texture<TYPE, cudaTextureType2D> TEXTURE_PYRAMID(NAME); \
    CudaImagePyramidHost NAME(cudaTextureType2D, sizeof(TYPE), #NAME, &TEXTURE_PYRAMID(NAME));
#define PYRAMID_FETCH(NAME,X,Y) tex2D(TEXTURE_PYRAMID(NAME), (X) + 0.5, (Y) + 0.5)

#define CUDA_IMAGE_PYRAMID_LAYERED(TYPE,NAME) \
    texture<TYPE, cudaTextureType2DLayered> TEXTURE_PYRAMID(NAME); \
    CudaImagePyramidHost NAME(cudaTextureType2DLayered, sizeof(TYPE), #NAME, &TEXTURE_PYRAMID(NAME));
#define PYRAMID_FETCH_LAYER(NAME,X,Y,LAYER) tex2DLayered(TEXTURE_PYRAMID(NAME), (X) + 0.5, (Y) + 0.5,(LAYER))
