...
class CudaImageBufferDevice
{
    public:
        ...
        __device__ T pixel(int x, int y, int line = 0)
        {
...
#else
        __device__ T pixel(int x, int y)
        {
#endif
            return *((T*)((char*)_storage + y * _pitch) + x);
        }

        __device__ T pixelWrap(int x, int y)
        {
...
#ifdef CUDA_IMAGE_BUFFER_DEVICE_DEBUG
        __device__ void setPixel(const T& value, int x, int y, int line = 0)
        {
            if (x < 0 || x >= _width || y < 0 || y >= _height) {
                printf("Out of bounds access line %d, %d %d (%d %d)\n", line, x, y, _width, _height);
                return;
...
#else
        __device__ void setPixel(const T& value, int x, int y)
        {
#endif
            T* p = (T*)((char*)_storage + y * _pitch) + x; *p = value;
        }
	__device__ T at (int i, int level) {
            int y = floorf((float)i / (float)width(level));
            int x = (i - y*width(level));
	    return pixel(x,y);
        }

        __device__ void atomicAddition(const T& value, float x, float y)
        {
        ...
        __device__ bool inBounds(int x, int y, int level, int border = 0) {
            return x >= border && y >= border && x < width(level) - border && y < height(level) - border;
        }

        __device__ int width(int level = 0) { return _width >> level; }
        __device__ int height(int level = 0) { return _height >> level; }
...
class CudaImageBufferHost
{
	...
	const QVector<T>& hostCache() { updateHostCache(); return _host_cache; }
        ...
        __host__ __device__ CudaImageBufferDevice<T> deviceType()
        {
        ...
        __host__ __device__ operator CudaImageBufferDevice<T>() { return deviceType(); }
